<?php
	...
	/* Localidad en la base de datos es ciudad. */
	if (tieneCreditos()){
		$link = conectar();	
		if (mysqli_query($link, "UPDATE credito SET cantidad = cantidad - 1 WHERE id_usuario='$_SESSION[id]'")){
			if(!(empty($_FILES['foto']['tmp_name'])) && (file_exists($_FILES['foto']['tmp_name']))) {
				...
				mysqli_query($link, "INSERT INTO favor (titulo,descripcion,fechalimite,ciudad,id_usuario,id_categoria,activo,foto) VALUES ('$_POST[titulo]','$_POST[descripcion]','$_POST[fecha]','$_POST[loc]','$_SESSION[id]','$_POST[categoria]',1,'$nombretipoimg')");
			}
			else {
				mysqli_query($link, "INSERT INTO favor (titulo,descripcion,fechalimite,ciudad,id_usuario,id_categoria,activo,foto) VALUES ('$_POST[titulo]','$_POST[descripcion]','$_POST[fecha]','$_POST[loc]','$_SESSION[id]','$_POST[categoria]',1,'default.png')");
			}
			mysqli_close($link);
