			...
			<div class="w3-row w3-padding-64">
				<div class="w3-container">
					...
					<?php
						...
						$consulta = "SELECT favor.id AS idfavor, favor.titulo, favor.id_categoria, favor.descripcion, favor.ciudad, favor.foto, favor.fechalimite, favor.id_usuario, favor.activo, usuario.nombre, usuario.apellido, usuario.puntaje, usuario.id AS idusuario, postula.id AS idpostula, postula.descripcion, postula.id_usuario FROM favor LEFT JOIN postula ON (favor.id = postula.id_favor) LEFT JOIN usuario ON (postula.id_usuario = usuario.id) WHERE favor.id_usuario = '$_SESSION[id]' AND (CURDATE() <= favor.fechalimite) AND (favor.activo = 1) ORDER BY usuario.puntaje DESC, favor.titulo";
